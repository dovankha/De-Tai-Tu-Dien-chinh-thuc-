...
int main()
{
	...
	cout<<"Danh sach tu co trong tu dien: "<<endl;
	for (NODE *k = l.pHead; k!=NULL; k=k->pNext)
	{
		k->data.showTu();
	} 
			...
			else
			{
				...
				while (true)
				{
					...
					{ 
						...
						if (loai.length()==0)
						{
							cout<<"Hay nhap loai tu!"<<endl<<endl;			 
						}
					}
					Sloai = Sloai + loai + ';';
					while (nghia.length()==0) 
					{ 
						cout << "Nhap nghia cua tu: ";
						getline(cin, nghia);
						if (nghia.length()==0)
						{
							cout<<"Hay nhap nghia cua tu!"<<endl<<endl;			 
						}
					} 
